...
class PatternController:
  ...
  def __init__(
    ...
    # Model: 8-dim features → 6 buckets (A–F)
    self.model = AccessPatternClassifier(input_dim=8, num_classes=6)
    ...
    self.current_policy: str = "lru"  # safe default
  ...
  def _pick_bucket_from_probs(self, probs: torch.Tensor) -> int:
    ...
    if pred in (1, 5):  # B or F
      ...
      # Only trust B/F if they are clearly dominant.
      # Threshold can be tuned; start with 0.7.
      if pred_prob < 0.7:
        # Restrict prediction to A, C, D, E instead.
        allowed = [0, 2, 3, 4]
        sub = probs[allowed]
